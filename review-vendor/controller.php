<?php
	...
	$Tag = new Tag(URL);
	$head = $Tag->createHead(SITENAME." | Vendor's Questionnaire ", "nav-md view-staff-page", ['css' => ['css/nprogress.css']]);
	
	$menu = $Tag->createSideBar($PDO, $userDetails['email'], ['parent'=>'New Review', 'child'=>'']);
	...
	if($reviewees = $DbHandle->iRetrieveData(__LINE__, [Functions::getViewer($userDetails['authority'])=>"yes"])){
		foreach ($reviewees as $aReviewee) {
			$reviewers = Functions::getReviewers($aReviewee, $User);
          ...
          <td>
          	<a href='".URL."view-vendor-general/?vendor={$aReviewee['vendor_id']}&review={$aReviewee['id']}' class='btn btn-primary btn-sm'>Questionaire</button>
          </td>
        </tr>
			";
		}
