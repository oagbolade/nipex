<?php
	...
	$pageName = "My Reviewed Audit Report";
	...
	$Tag = new Tag(URL);
	$head = $Tag->createHead(SITENAME." | My Audit Review ", "nav-md view-staff-page", ['css' => ['css/nprogress.css']]);
	
	$menu = $Tag->createSideBar($PDO, $userDetails['email'], ['parent'=>'My Audit Report', 'child'=>'View Staff']);
	...
	if($reviewees = $DbHandle->iRetrieveData(__LINE__, [getViewer($userDetails['authority'])=>$userDetails['id']])){
		foreach ($reviewees as $aReviewee) {
			$reviewers =Functions::getReviewers($aReviewee, $User, false, true);
